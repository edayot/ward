...
/**
 * Asserts that items matching a predicate exist in entity inventory or block container slots.
 */
				...
				.then(Commands.literal("entity").then(Commands.argument("entities", EntityArgument.entities())
						.then(Commands.argument("slots", SlotsArgument.slots())
								.then(Commands.argument("predicate", ItemPredicateArgument.itemPredicate(context.buildContext()))
										.executes(ctx -> assertItemsEntity(ctx, context))))))
				.then(Commands.literal("block").then(Commands.argument("pos", BlockPosArgument.blockPos())
						.then(Commands.argument("slots", SlotsArgument.slots())
								.then(Commands.argument("predicate", ItemPredicateArgument.itemPredicate(context.buildContext()))
										.executes(ctx -> assertItemsBlock(ctx, context)))))));
	}

	private static int assertItemsEntity(CommandContext<CommandSourceStack> context, Context assertion) throws CommandSyntaxException {
		...
		return assertion.apply(() -> {
			...
			return new TestExecutor.AssertResult(count, negated -> {
				String key = Assertions.getTranslationKey("items", negated);
				String input = Assertions.getRawArgument(context, "predicate");
				return Component.translatable(key, input, count);
			});
		});
	}

	private static int assertItemsBlock(CommandContext<CommandSourceStack> context, Context assertion) throws CommandSyntaxException {
		SlotRange slots = SlotsArgument.getSlots(context, "slots");
		Predicate<ItemStack> predicate = ItemPredicateArgument.getItemPredicate(context, "predicate");
