...
Release PRs can also be opened from the Actions UI: the *release-pr*
workflow runs the bump, opens the PR, and auto-merges it once CI is green —
merging releases.

Release notes are generated by GitHub from the pull requests merged since
the previous release (and reused as the Modrinth changelog) — give your PRs
titles that read well to users.
...
`main` targets the newest supported Minecraft line; older lines live on
maintenance branches named after the line (`26.1`, `26.2`, ...). When a new
Minecraft version comes out — snapshots included — the *watch* workflow
hands it to *release-pr*, which retargets the toolchain (minecraft, loader,
fabric-api) and opens a PR. CI gates it: a working Ward auto-merges and
releases the compatibility build (snapshots publish as beta); a broken one
stays open and red for a maintainer. When a merge moves `main` to a new
line, the *maintenance* workflow snapshots the old line into its branch —
pointing at the last commit that still targeted it — and `main` moves on;
maintenance branches are updated manually from then on.
